...
test('should hide characters at the start of the string', () => {
...
test('should hide characters at the end of the string', () => {
  const value = 'password123';
  const expected = '********123';
...
test('should throw an error if the char length is greater than 1', () => {
  const value = 'password123';
  const char = '**';
...
test('should throw an error if the nbChars is greater than the value length', () => {
  const value = 'password123';
  const nbChars = 20;
  const expectedError = new Error(
    'The number of characters to extract from the former string exceed the string lenght'
  );
